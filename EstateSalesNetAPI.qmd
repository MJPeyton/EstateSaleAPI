...
https://www.estatesales.net/token
...
query <- 
    GET(
        url = glue("https://www.estatesales.net/api/sale-details?bypass=bycoordinatesanddistance:{lat}_{lon}_{distance}&include=saleschedule&select=id,url,stateCode,cityName,postalCodeNumber,primaryMetroAreaId,latitude,longitude,utcOffset,observesDaylightSavingTime,type,isMarketplaceSale,isLocallyFeatured,isRegionallyFeaturedThisWeek,isNationallyFeaturedThisWeek,firstUtcStartDate,firstLocalStartDate,lastUtcEndDate,lastLocalEndDate,activeLocalStartDate,tomorrowActiveLocalStartDate,utcDateFirstPublished,utcShowAddressAfter,orgPackageType,saleSchedule&explicitTypes=DateTime"))
...
```{r}
 ...
 geocode(responseOutput, lat = "lat", long = "lon", return_addresses = TRUE)
